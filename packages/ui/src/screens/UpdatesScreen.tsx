  ...
  return (
    <section className="product-screen updates-screen">
      ...
      <div className="updates-list" role="feed" aria-label="版本更新紀錄">
        {entries.map((entry) => {
          const isDevelopment = entry.status === 'development';
          return (
            <article
              key={entry.version}
              className={`update-card${isDevelopment ? ' update-card--current' : ''}`}
            ...
            >
              <div className="update-card__header">
                <div className="update-card__version-row">
                  ...
                  <span
                    className={`update-badge${isDevelopment ? ' update-badge--current' : ''}`}
                  >
                    {isDevelopment ? '目前開發版本' : '已發布'}
                  </span>
                </div>
