...
export function ResetCreditsScreen({ items, now = new Date() }: { items: UsageSnapshot[]; now?: Date }) {
  ...
  return <section className="product-screen reset-screen">
    ...
    <section className="reset-hero"><span className="reset-hero__icon" aria-hidden="true">↻</span><div><span>目前可用</span><strong>{resolved.availableCount}</strong><small>張 Reset Credit{resolved.expiredCount > 0 ? `（另有 ${resolved.expiredCount} 張已過期）` : ''}</small></div></section>
    <section className="section"><div className="section-title-row"><div><p className="screen-eyebrow">Read-only</p><h2>可用額度</h2></div></div><ResetCreditList resource={resource} now={now} /></section>
