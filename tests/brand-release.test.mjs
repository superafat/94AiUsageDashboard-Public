...
test('version sources cannot silently drift from root/workspace package version 0.1.4', async () => {
  ...
  assert.match(brandContent, /version: ['"`]0\.1\.4['"`][\s\S]*?status: ['"`]development['"`]/, 'v0.1.4 must be marked development until a formal public Release exists');
  const readme = fs.readFileSync(path.resolve('README.md'), 'utf8');
  assert.match(readme, /最新正式公開 Release.*v0\.1\.3/i);
  assert.match(readme, /目前.*開發版本.*v0\.1\.4/i);
});
